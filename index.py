...
def reset():  
    ...
    mb.showinfo("Girişleri täzeden düzmek", "Ähli ýazgylar üstünlikli täzeden düzüldi!")  
...
def check_for_errors():  
    ...
    if (birth_year_field.get() == "" or birth_month_field.get() == "" or birth_day_field.get() == "" or given_year_field.get() == "" or given_month_field.get() == "" or given_day_field.get() == ""):  
        mb.showerror("Giriş säwligi", "Nädogry format! Gaýtadan synanyşyň.")  
    ...
    if val == -1:  
        ...
        try:  
            ...
            if (birth_date < given_date):  
                ...
                age_var.set(str(age) + " Ýaşynda")  
  
            else:  
                mb.showerror("Giriş säwligi", "Sene tapawudy uly!")   
                reset_entries()  
  
        except ValueError:   
            mb.showerror("Aralykda", "Giriş senesi çäkden çykýar.")  
            reset_entries()  
    
if __name__ == "__main__":  
  
    gui_root = Tk()  
    gui_root.title("Ýaşyňyzy-hasaplamak")  
    gui_root.geometry("600x450+650+250")   
    gui_root.resizable(0, 0)   
    header_frame = Frame(gui_root)  
    entry_frame = Frame(gui_root)  
    result_frame = Frame(gui_root)  
    ...
    header_label = Label(  
        header_frame,  
        text = "Ýaşyňyzy-hasaplaýan-platforma",  
